...
init : (Model, Cmd Msg)
init = ( Model (List.map (\x -> 0) <| List.range 0 ( n - 1 ) ) , Cmd.none )

type Msg = Drop Int | Tick Time

update : Msg -> Model -> (Model, Cmd Msg)
update msg model  =
  case msg of
      Tick t -> ( model , Random.generate Drop <| Random.int 0 (n-1)  )
      Drop x ->
...
h : (Int, Int) -> Svg Msg
h (a,b) = rect [ width "20", height "20", x <| toString (25*b) , y <| toString (25*(n-1) - (25*a)) , fill "#EB895B" ] []

-- some style problems.  why not just have a fixed set of pixels (e.g. 8 by 10) and change the colors?
view : Model -> Html Msg
view model =
  div [ style [ ("margin", "5px") ] ]
  [ svg [ height <| toString (n*25), width <| toString (n*25)] <| List.map h <| List.concat <| List.map (\(a,b) -> List.map (\c -> (c,a) )  (List.range 0 (b-1)  ) ) <| List.map2 (,) (List.range 0 n) model.stack
  , div [] <| List.map g model.stack
--  , div [] <| List.map f <| List.range 1 n
  ]
