...
getR model =
  if model.p then
    toHex  <| get ( model.x*4 + 500*4*model.y ) model.img
  else
    "255"
...
getG model =
  if model.p then
    toHex  <| get ( model.x*4 + 500*4*model.y + 1 ) model.img
  else
    "255"
...
getB model =
  if model.p then
    toHex  <| get ( model.x*4 + 500*4*model.y + 2 ) model.img
  else
    "255"
...
view model =
  ...
  div [ Html.width 500 ] [
    div [   ]
      ...
      , div [ style [ ("width", "100px"), ("text-align", "left"), ("display", "inline-block")] ] [text <| getColor model  ]
